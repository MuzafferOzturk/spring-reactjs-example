...
class UnitTestExample {
    ...
    public void getAllUser(){
        when(usersRepository.findAll()).thenReturn(Collections.singletonList(new UserDocument("t","p")));
        assertEquals(1, auth.getAllUser().size());
    }

    @Test
    @Order(2)
    @DisplayName("Given Test")
    public void getAllUser2(){
        given(usersRepository.findAll()).willReturn(Collections.singletonList(new UserDocument("t","p")));
        assertEquals(1, auth.getAllUser().size());
    }
